...
def move_servos(current_x, current_y, target_x, target_y, slow_speed=0.1, fast_speed=0.1):
    ...
    if abs(dx) < 10 and abs(dy) < 10:
        speed = slow_speed  # Slow down when close to the target
    else:
        speed = fast_speed  # Move faster when far from the target

    # Move the servos towards the target position
    new_x = current_x + int(dx * 0.1)
    new_y = current_y + int(dy * 0.1)
    
    # Constrain servo positions to valid range (0-180)                BOTH NEED EDIT
    new_x = max(0, min(90, new_x))
...
def track_faces():
    ...
    while cap.isOpened():
        ...
        for (x, y, w, h) in faces:
            ...
            current_x, current_y = move_servos(current_x, current_y, x_center, y_center, slow_speed=0.1, fast_speed=0.1)
